document.addEventListener('DOMContentLoaded', () => {
    ...
    var stepTimer;
    var crazyModeOn = false;
    var crazyModeSpeed = 300;
    ...
    var chboxCrazyModeOn = document.getElementById('crazyModeOn');
    ...
    function gameStep(isDown) {
        ...
        if (canMove(gameBoard.nextPosition)) {
           ...
           //CrazyModeON - скорость падения фигур меняется каждый раз неожиданно, что держит в напряжении,
           // каждый раз неожидаешь с какой скоростью будет падать фигура. Ничто не держит так в напряжении, 
           // как неожиданный поворот
            if (chboxCrazyModeOn.checked) {
                gameBoard.stepDelay = Math.floor(Math.random() * (crazyModeSpeed - 10 + 1)) + 10;
            } else {
                gameBoard.stepDelay = Math.max(100, 1000-10*gameBoard.finishedLines);
               // alert(gameBoard.stepDelay)
            }
